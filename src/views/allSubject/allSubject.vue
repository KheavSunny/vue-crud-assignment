    ...
    <div class="grid gap-4 grid-cols-2">
      <div class="pl-10 w-70%">
        <el-input :class="subject.title || !errorMessage ? 'mb-2' : ''" v-model="subject.title"
          placeholder="Input Subject"></el-input>
        <div v-if="!subject.title && errorMessage" class="text-red-600 text-sm mb-2">Please Input Subject</div>
        <el-input v-model="subject.description" placeholder="Input Description"></el-input>
        <div v-if="!subject.description && errorMessage" class="text-red-600 text-sm">Please Input Description</div>
        <el-button v-if="!showBtnEdit" @click="addSubject(subject)"
          class="bg-blue-500 text-white hover:bg-green-500 hover:text-white mt-2">Create</el-button>
        <el-button v-else @click.prevent="updateSubject(subject)"
          class="bg-blue-500 text-white hover:bg-green-500 hover:text-white mt-2">Update</el-button>
      </div>
    </div>
    <div class="grid gap-4 grid-cols-2 pl-10 pt-4">
      <div>
        <div class="header_subject">Subject List</div>
          ...
          <div v-if="getListSubjects">
            <div v-for="item in getListSubjects"
              ...
              <el-button @click="btnDelete(item.id)"
                class="block border-none hover:bg-red-600 hover:text-white text-red-600">Delete</el-button>
            </div>
          </div>
      ...
      <div>
        <div class="header_subject">Subject Details</div>
          ...
          <div class="pl-5 pt-2">
            ...
            <div>
              <el-button v-if="getSubjectDetail" type="warning"
                ...
                @click="editSubject(getSubjectDetail.id)">Edit</el-button>
            </div>
          </div>
        </div>
      </div>
